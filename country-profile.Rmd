...
medians <- read_csv("data/at_health_facilities.csv",
  ...
  janitor::clean_names() %>%
  group_by(iso3, year) %>%
  summarize(median_perc = median(age_15_17, age_20_34,
                                 na.rm = TRUE)) %>%
...
year <- single_iso %>%  
  distinct(year)
```
`r year$year` have data for `r my_country`.
...
medians %>%
  ...
  ggplot(aes(x = fct_reorder(iso3, median_perc),
             y = median_perc)) +
  geom_col() +
  geom_col(data = single_iso %>%
          filter(year == most_recent), 
          fill = "dodgerblue") +
  labs(x = NULL, 
       y = "HIV incidence (%)") +
